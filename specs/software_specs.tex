...
Using the camera the User can then select an origin point and place calibration grids \ref{fig:grid} within the \glsname{VR}. Then by saving these images and analyzed for lens distortion (provided by \glsname{OpenCV}). 
...
User will use an \ref{OpenCV} interface to retrieve distortion parameters.
User will analyse and derive the extrinsice and intrinsic values of the parameters. 

\subsection{Accuracy Modeling}

User will adjust distortion levels and reproject the selected calibration grids to selected poses. System will calculate difference between points on the actual and reporjected grid. 
...
User can also perform accuracy modeling between two cameras to decide which one to pick.

\subsection{Training} 

During planing the Trainer user will select student emails, and decide steps duration (changes in data models). The steps are create such that as models are changed (such as values confirmed for extrinsic matrix) as training steps. 
The Trainer user can then send a hyper text link via email to the student. The Student user will go to the link and follow the training steps, and filling out difficulty and written feed back. 
